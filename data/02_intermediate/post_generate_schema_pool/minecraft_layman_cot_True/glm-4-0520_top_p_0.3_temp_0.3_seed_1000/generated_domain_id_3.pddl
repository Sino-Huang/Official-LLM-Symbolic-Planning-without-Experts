(define (domain minecraft)
    ...
    (:action recall
        ...
        :precondition (and
            (equipped ?item ?agent)     ;; The agent is holding the item
        )
        :effect (and
            (not (at ?item ?location)) ;; If the item was in the world, it's no longer there
            (inventory ?item)          ;; The item is now in the inventory
            (handsfree ?agent)                ;; The agent's hands are free if they were holding the item
        )
    )
    (:action pick
