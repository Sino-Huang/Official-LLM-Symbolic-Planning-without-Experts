    ...
    (:action destroy-sword
        ...
        :effect (and
            ...
            (when (has-monster ?loc)
                  (is-destroyed ?loc)))  ; This assumes that if a monster is present, the hero's action leads to the destruction of the hero's current location, which could be a simplification.
    )
    (:action move
        :parameters (?from - cells ?to - cells)
        :precondition (and
