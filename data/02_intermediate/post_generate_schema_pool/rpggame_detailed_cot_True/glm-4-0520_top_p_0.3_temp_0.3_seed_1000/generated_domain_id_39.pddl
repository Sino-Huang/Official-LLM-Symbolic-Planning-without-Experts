(define (domain rpggame)
    ...
    (:action destroy-sword
        ...
        :effect (and
            (not (holding ?s))
            (is-destroyed ?s)
            ; Assuming that the following effects are represented by other predicates not listed
            ; (triggered-trap) ; If the hero is in a room with a trap
            ; (monster-attacks) ; If the hero is in a room with a monster
        )
    )
    (:action move
        :parameters (?from - cells ?to - cells)
