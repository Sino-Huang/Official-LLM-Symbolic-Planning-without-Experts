(define (domain rpggame)
    ...
    (:action destroy-sword
        :parameters (?s - swords ?loc - cells ?s - swords ?loc - cells)
        :precondition (and
            (holding ?s)
            (at-hero ?loc)
        )
        :effect (and
            (not (holding ?s))
            (is-destroyed ?s)
            ;; Additional effects for triggering a trap or provoking a monster would be needed here,
            ;; but are not included as they are not specified in the available predicates.
        )
    )
    (:action move
        :parameters (?from - cells ?to - cells)
