(define (domain rpggame)
    ...
    (:action destroy-sword
        ...
        :effect (and
            (not (holding ?s))
            (is-destroyed ?s)
            ;; The following are not explicit effects of this action but potential
            ;; consequences of the state change that should be considered in the overall plan:
            ;; (has-monster ?loc) -> monster attacks
            ;; (has-trap ?loc) -> trap is triggered
        )
    )
    (:action move
        :parameters (?from - cells ?to - cells)
