(define (domain rpggame)
    ...
    (:action destroy-sword
        ...
        :effect (and
            (not (holding ?s))
            (is-destroyed ?s)
            ; Note: The action does not explicitly include effects for triggering traps or attracting monsters,
            ; as we do not have specific predicates for these interactions in the available list.
        )
    )
    (:action move
        :parameters (?from - cells ?to - cells)
