(define (domain rpggame)
    ...
    (:action destroy-sword
        ...
        :effect (and (not (holding ?s)) (is-destroyed ?s) (when (has-trap ?loc) (not (trap-disarmed ?loc))) (when (has-monster ?loc) (is-destroyed ?loc)))
    )
     (:action disarm-trap
     ...
     (:action move-to-monster
        ...
        :precondition (and (at-hero ?loc) (connected ?loc ?monster-loc) (not (is-destroyed ?monster-loc)) (has-monster ?monster-loc))
        :effect (and (not (at-hero ?loc)) (at-hero ?monster-loc))
    )
     (:action move-to-trap
        :parameters (?from - cells ?to - cells)
        :precondition (and (at-hero ?from) (connected ?from ?to) (not (is-destroyed ?to)) (has-trap ?to) (not (trap-disarmed ?to)))
